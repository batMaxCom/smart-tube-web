...
Приложение умеет запускаться как HTML5-виджет в оболочке MSX (TV-лаунчеры с MSX-движком).

| Endpoint            | Назначение                                                    |
| ------------------- | ------------------------------------------------------------- |
| `GET /msx/start.json` | точка входа, добавляется в MSX как «Start parameter»          |
| `GET /msx/menu.json`  | меню приложения: «Смотреть», «О проекте»                     |
| `GET /msx/home.json`  | домашний экран MSX со ссылками на поиск и настройки           |
| `GET /msx/about.json` | информация о приложении (правила `link:` / `content:`)        |

Как подключить:

1. В MSX выбрать «Add app / Добавить приложение» и указать адрес стартового параметра:
   `http://<хост>:8085/msx/start.json` (тот же URL показан в «Настройки → Media Station X»).
2. Запустить приложение из меню MSX. Ссылки ведут на `http://<хост>:8085/?msx=1`, поэтому адрес
   для MSX должен быть тем же, что и для обычного входа с ТВ/телефона.
3. Внутри приложения в шапке появляется кнопка выхода — она закрывает HTML5-приложение и
   возвращает управление оболочке MSX.

Адрес в JSON-файлах собирается из заголовка `Host` (nginx проксирует `$http_host`, порт сохраняется).
Если запускать через другой домен/reverse-proxy, задайте `MSX_PUBLIC_URL` (например
`http://tv.local:8085`) — тогда он будет подставляться вместо заголовка `Host`.

## Мобильный интерфейс

- Контролы плеера не зависят от hover: показываются по тапу/клавише и гаснут через 3.5 секунды
  воспроизведения; на паузе остаются на месте.
- Шапка с горизонтальной прокруткой, крупные тап-таргеты, ползунки под палец, safe-area для
  вырезов и жестовой полосы, экранная клавиатура переносится на узких экранах.
- Проверки: `e2e/mobile.spec.ts` (тач-контекст 390×844).
